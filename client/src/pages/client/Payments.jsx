  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-2">
        ...
        <span className="text-xs font-bold text-slate-500 uppercase tracking-wide">Loading Payments ledger...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900">Payments & Escrow</h1>
        <p className="text-xs text-slate-500 mt-1">Track platform escrow transactions, fund pending milestones, and view payouts history.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        <div className="bg-white p-4 md:p-6 rounded-2xl border border-surface-border flex items-center gap-4">
          ...
          <div>
            <span className="block text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-wide">Total Spent</span>
            <span className="text-lg md:text-xl font-extrabold text-slate-900">₹{totalSpent.toLocaleString('en-IN')}</span>
          </div>
        </div>

        <div className="bg-white p-4 md:p-6 rounded-2xl border border-surface-border flex items-center gap-4">
          <div className="p-3 rounded-xl bg-orange-600/10 border border-orange-600/20 text-orange-600 shrink-0">
            <CreditCard className="h-5 w-5" />
          </div>
          <div>
            <span className="block text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-wide">In Escrow</span>
            <span className="text-lg md:text-xl font-extrabold text-slate-900">₹{inEscrow.toLocaleString('en-IN')}</span>
          </div>
        </div>

        <div className="bg-white p-4 md:p-6 rounded-2xl border border-surface-border flex items-center gap-4">
          <div className="p-3 rounded-xl bg-orange-500/10 border border-orange-500/20 text-purple-400 shrink-0">
            <Award className="h-5 w-5" />
          </div>
          <div>
            <span className="block text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-wide">Gigs Paid</span>
            <span className="text-lg md:text-xl font-extrabold text-slate-900">{totalGigsPaid} Contracts</span>
          </div>
        </div>
        ...
        <div className="bg-white p-4 md:p-6 rounded-2xl border border-surface-border flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-lg md:text-xl font-extrabold text-slate-900">₹{thisMonthSpent.toLocaleString('en-IN')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <h2 className="text-md font-bold text-slate-900 uppercase tracking-wider">Funds in Escrow</h2>
        {escrowPayments.length === 0 ? (
          <p className="text-xs text-slate-500 font-bold py-2">No active escrow transactions.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {escrowPayments.map((p) => (
              <div key={p._id} className="bg-white p-6 rounded-2xl border border-surface-border flex flex-col justify-between space-y-4 hover:border-[rgba(255,255,255,0.08)] transition-colors">
                <div className="space-y-2">
                  <div className="flex justify-between items-start gap-2">
                    <h3 className="text-sm font-bold text-slate-900 leading-snug line-clamp-1">{p.gig?.title || 'Contract Payment'}</h3>
                    <span className="px-2 py-0.5 rounded bg-orange-600/15 text-orange-600 text-[9px] font-extrabold uppercase tracking-wide shrink-0">
                ...
                {p.status === 'escrow' && (
                  <button
                    ...
                    className="w-full py-2 bg-[#10B981] hover:bg-[#10B981]/90 text-slate-900 font-bold text-xs rounded-xl cursor-pointer transition-colors shadow-lg shadow-green-500/10 text-center"
                  >
                    Release Payment
                  </button>
                )}
                ...
                {payments.map((p) => {
                  ...
                  return (
                    <tr key={p._id} className="hover:bg-white/[0.01]">
                      <td className="px-6 py-4 font-bold text-slate-900">{p.gig?.title || 'Gig Contract'}</td>
                      ...
                      <td className="px-6 py-4 text-slate-500">₹{p.platformFee.toLocaleString()}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        {p.status === 'escrow' ? (
                          ...
                          <button
                            disabled
                            className="px-3 py-1.5 bg-surface-muted border border-surface-border text-[#475569] font-bold text-[10px] rounded-lg cursor-not-allowed"
                          >
                            Raise Dispute
                          </button>
                        )}
