  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-2">
        ...
        <span className="text-xs font-bold text-slate-500 uppercase tracking-wide">Loading Checkout...</span>
      </div>
    );
  }
  ...
  if (existingPayment && existingPayment.status !== 'refunded') {
    return (
      <div className="max-w-xl mx-auto py-12 text-center space-y-4">
        ...
        <h2 className="text-xl font-extrabold text-slate-900">Payment Already Made</h2>
        <p className="text-xs text-slate-500">
          A payment of {fmt(existingPayment.amount)} has already been submitted for this proposal.
          Status: <strong className="text-slate-900 capitalize">{existingPayment.status}</strong>
        </p>
        <button
          onClick={() => navigate('/client/payments')}
          className="px-6 py-2.5 bg-orange-600 text-slate-900 text-xs font-bold rounded-xl hover:opacity-90 transition-all cursor-pointer"
        >
          View Payments
        </button>
      </div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in duration-200">
      ...
      <div>
        ...
        <h1 className="text-2xl font-extrabold text-slate-900">Milestone Funding</h1>
        <p className="text-xs text-slate-500 mt-1">Fund project milestones securely. Funds are held in platform escrow until you approve deliverables.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-surface-border space-y-4">
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-surface-border/40 pb-2">Project Contract Summary</h2>
            
            <div className="space-y-1.5">
              <span className="text-[10px] text-slate-500 font-bold uppercase">Gig Listing</span>
              <p className="text-sm font-bold text-slate-900 leading-snug">{proposal.gig?.title}</p>
            </div>

            {milestone && (
              <div className="p-3 bg-surface-muted border border-surface-border rounded-xl space-y-1 text-xs">
                <span className="text-[10px] font-bold text-slate-500 uppercase">Funding Milestone</span>
                <p className="font-semibold text-slate-900">{milestone.title}</p>
                {milestone.description && <p className="text-[10px] text-slate-500">{milestone.description}</p>}
              </div>
            )}

            <div className="flex items-center gap-3 border-t border-surface-border/40 pt-4">
              <img
              ...
              />
              <div>
                <span className="block text-[9px] text-slate-500 font-extrabold uppercase">Assigned Freelancer</span>
                <span className="text-xs text-slate-900 font-bold">{proposal.freelancer?.name}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-surface-border space-y-4">
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-surface-border/40 pb-2">Amount Breakdown</h2>
            
            <div className="space-y-3.5 text-xs">
              <div className="flex justify-between">
                <span className="text-slate-500">Gig Amount</span>
                <span className="font-bold text-slate-900">₹{freelancerNet.toLocaleString('en-IN')}</span>
              </div>
              <div className="flex justify-between items-center">
                <div>
                  <span className="text-slate-500 block">Platform Service Fee (10%)</span>
                  <span className="text-[9px] text-slate-500 block mt-0.5">Secure payment escrow protection fee</span>
                </div>
                <span className="font-medium text-slate-500">₹{platformFee.toLocaleString('en-IN')}</span>
              </div>
              
              <div className="flex justify-between border-t border-surface-border/40 pt-4 text-sm font-black text-slate-900">
                <span>Total to Pay</span>
                <span className="text-lg text-orange-600">₹{totalAmount.toLocaleString('en-IN')}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-surface-border space-y-4">
            <div className="flex items-center gap-2.5 text-orange-600 border-b border-surface-border/40 pb-3">
            ...
            <button
              ...
              disabled={paying}
              className="w-full py-3 bg-gradient-orange hover-glow-orange text-slate-900 text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
            >
