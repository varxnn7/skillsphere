...
const UserRow = ({ user, selected, onSelect, onSuspend, onVerify, onDelete }) => {
  ...
  return (
    <tr className="hover:bg-white/[0.01] text-slate-300 border-b border-surface-border/40 text-xs">
      ...
      <td className="px-6 py-4">
        <span className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wide ${
          ...
          user.role === 'freelancer' ? 'bg-orange-500/10 text-purple-400' :
      ...
      <td className="px-6 py-4">
        {user.role === 'freelancer' ? (
          <span className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wide ${
            user.isVerified ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-surface-muted text-slate-500'
          }`}>
        ...
        <div className="flex justify-end gap-2">
          {user.role === 'freelancer' && !user.isVerified && (
            <button
              ...
              className="p-1.5 rounded-lg bg-surface-muted border border-surface-border hover:border-green-500/40 text-slate-500 hover:text-green-400 cursor-pointer transition-colors"
          ...
          {user.isSuspended ? (
            ...
            user.role !== 'admin' && (
              <button
                ...
                className="p-1.5 rounded-lg bg-surface-muted border border-surface-border hover:border-red-500/40 text-slate-500 hover:text-red-400 cursor-pointer transition-colors"
